...
#define DEFINE_SERIALIZE_CONTAINER(ContainerType, CONTAINER_FIELDS)                                     \
    ssz_error_t serialize_##ContainerType(const ContainerType *obj, uint8_t *out_buf, size_t *out_size) \
    {                                                                                                   \
...
#define SERIALIZE_VECTOR_FIELD(obj, offset, field, field_size, ser_func)         \
    ...
    {                                                                            \
        ...
        size_t tmp_size = (field_size);                                          \
        ssz_error_t err_local = ser_func((obj)->field, element_count, out_buf + (size_t)(offset), &tmp_size);
...
#define SERIALIZE_VECTOR_ARRAY_FIELD(obj, offset, field, element_size, count, ser_func) \
    ...
    for (size_t _i = 0; _i < (count); _i++)
    {
        ...
        ssz_error_t err_local = ser_func((obj)->field[_i], element_count, out_buf + (size_t)(offset), &tmp_size);
...
#define SERIALIZE_BITVECTOR_FIELD(obj, offset, field, bits)  \
    ...
    {                                                        \
        ...
        ssz_error_t err_local = ssz_serialize_bitvector((obj)->field, (bits), out_buf + (size_t)(offset), &tmp_size);
...
#define SERIALIZE_BITLIST_FIELD(obj, offset, field, max_bits)                   \
    ...
    {                                                                           \
        ...
        ssz_error_t err_local =                                                 \
            ssz_serialize_bitlist((obj)->field.data, (obj)->field.length, out_buf + (size_t)(offset), &tmp_size);
...
#define SERIALIZE_CONTAINER_FIELD(obj, offset, field, container_ser_func, field_size) \
    ...
    {                                                                                 \
        size_t tmp_size = (field_size);                                               \
        ssz_error_t err_local = container_ser_func(&(obj)->field, out_buf + (size_t)(offset), &tmp_size);
...
for (uint32_t _i = 0; _i < _num_elements; _i++)
{
    ...
    _tmp_size = SSZ_BYTE_SIZE_OF_UINT32;
    _err = ssz_serialize_uint32(&_this_elem_offset,
...
{
    size_t _elem_ser_size = 0;
    ssz_error_t _err = container_ser_func(&(obj)->field.data[_i], out_buf + (size_t)(_cur_offset), &_elem_ser_size);
...
#define DEFINE_DESERIALIZE_CONTAINER(ContainerType, CONTAINER_FIELDS)                                        \
    ssz_error_t deserialize_##ContainerType(const unsigned char *data, size_t data_size, ContainerType *obj) \
    {                                                                                                        \
        (void)data_size;                                                                                     \
        ssz_offset_t offset = 0;                                                                             \
        CONTAINER_FIELDS                                                                                     \
...
#define DESERIALIZE_VECTOR_FIELD(obj, offset, field, deserialize_func)                               \
    do                                                                                               \
    {                                                                                                \
        ssz_error_t err_local = deserialize_func(data + (size_t)(offset),                            \
                                                 sizeof((obj)->field),                               \
...
ssz_error_t err_local =
    ssz_deserialize_bitlist(data + (size_t)(offset_start), (field_size), (max_bits), (obj)->field.data, &actual_count);
...
ssz_error_t err_local =
    deserialize_func(data + (size_t)(offset_start), (list_size), (max_length), (obj)->field.data, &actual_count);
...
for (uint32_t _i = 0; _i < _num_elements; _i++)
{
    ...
    ssz_error_t _err = deserialize_func(_base_ptr + _elem_rel_offset, _elem_total_size, &((obj)->field.data[_i]));
