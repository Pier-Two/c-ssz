...
/*
 * Deserializes a container with field_count fields. We do a single pass to read or skip
 * offsets for variable fields, then a second pass to copy the actual slices. 
 */
...
{
    if (!buffer || !field_is_variable_size || !field_sizes || !out_container_data || field_count == 0)
    {
        return SSZ_ERROR_DESERIALIZATION;
    }
    ...
    if (fixed_region_size > buffer_size)
    {
        return SSZ_ERROR_DESERIALIZATION;
    }
    uint32_t *offsets = NULL;
    if (variable_count > 0)
    {
        offsets = (uint32_t *)malloc(variable_count * sizeof(uint32_t));
        if (!offsets)
        {
            return SSZ_ERROR_DESERIALIZATION;
        }
    }
    size_t buf_fixed_pos = 0;
    uint8_t *container_ptr = (uint8_t *)out_container_data;
    size_t container_offset = 0;
    size_t var_idx = 0;

    /*
     * First pass: read the "fixed region." For variable fields, that means reading offsets
     * into offsets[var_idx]. For fixed fields, copy the data directly.
     */
    for (size_t i = 0; i < field_count; i++)
    {
        if (field_is_variable_size[i])
        {
            if (!read_offset_le(buffer, buffer_size, buf_fixed_pos, &offsets[var_idx]) ||
                !check_max_offset(offsets[var_idx]) ||
                offsets[var_idx] > buffer_size)
            {
                free(offsets);
                return SSZ_ERROR_DESERIALIZATION;
            }
            buf_fixed_pos += BYTES_PER_LENGTH_OFFSET;
            var_idx++;
        }
        else
        {
            size_t fs = field_sizes[i];
            if (buf_fixed_pos + fs > buffer_size)
            {
                free(offsets);
                return SSZ_ERROR_DESERIALIZATION;
            }
            memcpy(container_ptr + container_offset, buffer + buf_fixed_pos, fs);
            buf_fixed_pos += fs;
            container_offset += fs;
        }
    }
    ...
    /*
     * Enforce ascending offsets among variable fields.
     */
    for (size_t i = 1; i < var_idx; i++)
    {
        if (offsets[i] <= offsets[i - 1])
        {
            free(offsets);
    ...
    /*
     * Second pass: copy data slices for variable fields from the offset region, ensuring
     * each slice matches the declared field_sizes[i].
     */
    var_idx = 0;
    for (size_t i = 0; i < field_count; i++)
    {
        ...
        uint32_t start_off = offsets[var_idx];
        uint32_t end_off = 0;
        var_idx++;
        if (var_idx < variable_count)
        {
            end_off = offsets[var_idx];
        }
        else
        {
            end_off = (uint32_t)buffer_size;
        }
        if (end_off < start_off || end_off > buffer_size)
        {
            free(offsets);
        ...
        memcpy(container_ptr + container_offset, buffer + start_off, slice_len);
        container_offset += slice_len;
    }
    if (offsets)
    {
        free(offsets);
