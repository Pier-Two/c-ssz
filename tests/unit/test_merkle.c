...
static bool test_hash_tree_root_basic_types(void)
{
    ...
    ASSERT_ERR(ssz_hash_tree_root_uint128(value128, &root), SSZ_SUCCESS);
    ...
    ASSERT_ERR(ssz_hash_tree_root_uint256(value256, &root), SSZ_SUCCESS);
...
static bool test_hash_tree_root_bitvector_and_bitlist(void)
{
    ...
    ASSERT_ERR(ssz_hash_tree_root_bitvector(bits, sizeof(bits), 10u, NULL, &root_bitvector), SSZ_SUCCESS);
    ...
    ASSERT_ERR(ssz_hash_tree_root_bitlist(bits, sizeof(bits), 10u, 10u, NULL, &root_bitlist), SSZ_SUCCESS);

    ssz_chunk_t length_chunk = zero_chunk();
    write_u64_le(length_chunk.bytes, 10u);
    ssz_chunk_t expected_bitlist;
    ASSERT_ERR(ssz_hash_2to1(NULL, &expected_bitvector, &length_chunk, &expected_bitlist), SSZ_SUCCESS);
...
static bool test_hash_tree_root_list_variants_and_mix_in_length(void)
{
    ...
    ASSERT_ERR(ssz_hash_tree_root_list_fixed(fixed_elements, 3u, 6u, 8u, NULL, &fixed_root), SSZ_SUCCESS);
    ...
    ASSERT_ERR(ssz_hash_2to1(NULL, &fixed_data_root, &length_chunk, &expected_fixed_root), SSZ_SUCCESS);
    ...
    ASSERT_ERR(ssz_hash_tree_root_list_composite(3u, 4u, &codec, NULL, &composite_root), SSZ_SUCCESS);
...
static bool test_mix_in_helpers(void)
{
    ...
    ASSERT_ERR(ssz_mix_in_active_fields(&base, active_fields, sizeof(active_fields), NULL, &root),
               SSZ_SUCCESS);
...
static bool test_merkleize_progressive_cases(void)
{
    const ssz_chunk_t c0 = make_chunk(0x10u);
    const ssz_chunk_t c1 = make_chunk(0x30u);
    const ssz_chunk_t c2 = make_chunk(0x50u);

    ssz_chunk_t root;

    ASSERT_ERR(ssz_merkleize_progressive(NULL, 0u, NULL, &root), SSZ_SUCCESS);
    ASSERT_CHUNK_EQ(root, zero_chunk());

    ssz_chunk_t zero = zero_chunk();
    ssz_chunk_t expected_one;
    ASSERT_ERR(ssz_hash_2to1(NULL, &c0, &zero, &expected_one), SSZ_SUCCESS);

    ASSERT_ERR(ssz_merkleize_progressive(&c0, 1u, NULL, &root), SSZ_SUCCESS);
    ASSERT_CHUNK_EQ(root, expected_one);

    ssz_chunk_t z1;
    ASSERT_ERR(ssz_hash_2to1(NULL, &zero, &zero, &z1), SSZ_SUCCESS);

    ssz_chunk_t h12;
    ssz_chunk_t left_for_right;
    ssz_chunk_t right_branch;
    ssz_chunk_t expected;

    ASSERT_ERR(ssz_hash_2to1(NULL, &c1, &c2, &h12), SSZ_SUCCESS);
    ASSERT_ERR(ssz_hash_2to1(NULL, &h12, &z1, &left_for_right), SSZ_SUCCESS);
...
static bool test_progressive_hash_tree_root_variants(void)
{
    const root_entry_t container_entries[] = {
        {0u, make_chunk(0x01u)},
        {1u, make_chunk(0x21u)},
    ...
    const ssz_chunk_t container_roots[3] = {
        container_entries[0].root,
        container_entries[1].root,
        container_entries[2].root,
    };
    const uint8_t active_fields[] = {0x07u};

    ssz_chunk_t progressive_container_root;
    ssz_chunk_t progressive_container_roots_root;
    ASSERT_ERR(ssz_hash_tree_root_progressive_container(3u,
                                                        active_fields,
               ...
               SSZ_SUCCESS);
    ASSERT_ERR(ssz_hash_tree_root_progressive_container_roots(container_roots,
                                                              3u,
                                                              active_fields,
                                                              sizeof(active_fields),
                                                              NULL,
    ...
    {
        list_fixed_elements[i] = (uint8_t)(0x80u + i);
    }

    ssz_chunk_t progressive_list_fixed_root;
    ASSERT_ERR(ssz_hash_tree_root_progressive_list_fixed(
                   list_fixed_elements, 5u, 8u, NULL, &progressive_list_fixed_root),
               SSZ_SUCCESS);

    ssz_chunk_t packed_chunks[2];
    packed_chunks[0] = zero_chunk();
    packed_chunks[1] = zero_chunk();
    memcpy(packed_chunks[0].bytes, list_fixed_elements, 32u);
    memcpy(packed_chunks[1].bytes, list_fixed_elements + 32u, 8u);

    ssz_chunk_t expected_progressive_list_fixed_data;
    ssz_chunk_t expected_progressive_list_fixed;
    ...
    const root_entry_t list_entries[] = {
        {0u, make_chunk(0x11u)},
        {1u, make_chunk(0x22u)},
        {2u, make_chunk(0x33u)},
    };
    const root_map_ctx_t list_ctx = {
    ...
    const ssz_chunk_t list_roots[3] = {
        list_entries[0].root,
        list_entries[1].root,
        list_entries[2].root,
    };

    ssz_chunk_t progressive_list_composite_root;
    ssz_chunk_t progressive_list_roots_root;
    ASSERT_ERR(ssz_hash_tree_root_progressive_list_composite(3u,
                                                              &list_codec,
                                                              NULL,
                                                              &progressive_list_composite_root),
               SSZ_SUCCESS);
    ASSERT_ERR(ssz_hash_tree_root_progressive_list_roots(
                   list_roots, 3u, NULL, &progressive_list_roots_root),
               SSZ_SUCCESS);
    ASSERT_CHUNK_EQ(progressive_list_composite_root, progressive_list_roots_root);

    uint8_t bits[33];
    memset(bits, 0xAA, sizeof(bits));
    bits[32] = 0x0Au;

    ssz_chunk_t progressive_bitlist_root;
    ASSERT_ERR(ssz_hash_tree_root_progressive_bitlist(
                   bits, sizeof(bits), 260u, NULL, &progressive_bitlist_root),
               SSZ_SUCCESS);

    ssz_chunk_t bit_chunks[2];
    bit_chunks[0] = zero_chunk();
    bit_chunks[1] = zero_chunk();
    memcpy(bit_chunks[0].bytes, bits, 32u);
    memcpy(bit_chunks[1].bytes, bits + 32u, 1u);
...
static bool test_null_hash_fn_fallback_uses_default(void)
{
    ...
    ASSERT_ERR(ssz_merkleize(chunks, 2u, SSZ_NO_LIMIT, ssz_hash_default(), &root_explicit), SSZ_SUCCESS);
...
static bool test_merkle_additional_error_paths(void)
{
    ...
    ASSERT_ERR(ssz_hash_tree_root_uint128(NULL, &root), SSZ_ERR_INVALID_ARGUMENT);
    ASSERT_ERR(ssz_hash_tree_root_uint128((const uint8_t[16]){0u}, NULL), SSZ_ERR_INVALID_ARGUMENT);
    ASSERT_ERR(ssz_hash_tree_root_uint256(NULL, &root), SSZ_ERR_INVALID_ARGUMENT);
    ASSERT_ERR(ssz_hash_tree_root_uint256((const uint8_t[32]){0u}, NULL), SSZ_ERR_INVALID_ARGUMENT);
    ASSERT_ERR(ssz_hash_tree_root_boolean(2u, &root), SSZ_ERR_ENCODING_INVALID);

    ASSERT_ERR(ssz_hash_tree_root_bitvector(&one_byte, 1u, 1u, NULL, NULL), SSZ_ERR_INVALID_ARGUMENT);
    ...
    ASSERT_ERR(ssz_hash_tree_root_bitlist(&one_byte, 1u, 5u, 4u, NULL, &root), SSZ_ERR_LIMIT_EXCEEDED);
    ASSERT_ERR(ssz_hash_tree_root_bitlist(&one_byte, 1u, UINT64_MAX, SSZ_NO_LIMIT, NULL, &root),
               SSZ_ERR_OVERFLOW);
    ...
    ASSERT_ERR(ssz_mix_in_active_fields(NULL, (const uint8_t[]){0x01u}, 1u, NULL, &root),
               SSZ_ERR_INVALID_ARGUMENT);
    ASSERT_ERR(ssz_mix_in_active_fields(&root, (const uint8_t[33]){0u}, 33u, NULL, &root),
               SSZ_ERR_SCHEMA_INVALID);
    ASSERT_ERR(ssz_mix_in_active_fields(&root, NULL, 1u, NULL, &root), SSZ_ERR_INVALID_ARGUMENT);
    ASSERT_ERR(ssz_mix_in_active_fields(&root, (const uint8_t[]){0x01u}, 1u, NULL, NULL),
               SSZ_ERR_INVALID_ARGUMENT);
    ...
    ASSERT_ERR(ssz_hash_tree_root_progressive_list_fixed(&one_byte, 1u, 1u, NULL, NULL),
               SSZ_ERR_INVALID_ARGUMENT);
    ...
    ASSERT_ERR(ssz_hash_tree_root_progressive_bitlist(&one_byte, 1u, 1u, NULL, NULL),
               SSZ_ERR_INVALID_ARGUMENT);
    ASSERT_ERR(ssz_hash_tree_root_progressive_bitlist(&one_byte, 1u, UINT64_MAX, NULL, &root),
               SSZ_ERR_OVERFLOW);
...
static bool test_merkle_active_field_validation_errors(void)
{
    ssz_chunk_t root;
    const root_entry_t entries[] = {
        {0u, make_chunk(0x10u)},
...
static bool test_merkle_failure_propagation_paths(void)
{
    ...
    ASSERT_ERR(ssz_merkleize(merkle_chunks, 2u, SSZ_NO_LIMIT, &failing_hash, &root), SSZ_ERR_HASH_FAILURE);
    ...
    ASSERT_ERR(ssz_hash_tree_root_list_fixed(
                   ...
                   &root),
               SSZ_ERR_HASH_FAILURE);
    ASSERT_ERR(ssz_hash_tree_root_list_roots(merkle_chunks, 2u, SSZ_NO_LIMIT, &failing_hash, &root),
               SSZ_ERR_HASH_FAILURE);
    ...
    ASSERT_ERR(ssz_hash_tree_root_vector_composite(2u, &left_fail_codec, NULL, &root),
               SSZ_ERR_TYPE_MISMATCH);
    ASSERT_ERR(ssz_hash_tree_root_list_composite(2u, SSZ_NO_LIMIT, &left_fail_codec, NULL, &root),
               SSZ_ERR_TYPE_MISMATCH);
    ...
    ASSERT_ERR(ssz_hash_tree_root_vector_composite(2u, &right_fail_codec, NULL, &root),
               SSZ_ERR_TYPE_MISMATCH);
    ASSERT_ERR(ssz_hash_tree_root_union(1u, false, &right_fail_codec, NULL, &root),
               SSZ_ERR_TYPE_MISMATCH);

    const uint8_t active_fields[1] = {0x03u};
    ASSERT_ERR(ssz_hash_tree_root_progressive_container(
                   2u, active_fields, sizeof(active_fields), &right_fail_codec, NULL, &root),
               SSZ_ERR_TYPE_MISMATCH);
    ...
    ASSERT_ERR(ssz_hash_tree_root_progressive_bitlist(
                   ...
                   &root),
               SSZ_ERR_HASH_FAILURE);
    ASSERT_ERR(ssz_hash_tree_root_progressive_container_roots(
                   merkle_chunks, 2u, active_fields, sizeof(active_fields), &failing_hash, &root),
               SSZ_ERR_HASH_FAILURE);
    ASSERT_ERR(ssz_hash_tree_root_progressive_list_roots(merkle_chunks, 2u, &failing_hash, &root),
               SSZ_ERR_HASH_FAILURE);
...
int main(void)
{
    const test_case_t tests[] = {
        ...
        {"hash_tree_root_vector_fixed_pack_and_merkleize", test_hash_tree_root_vector_fixed_pack_and_merkleize},
        {"hash_tree_root_vector_composite_and_roots_match", test_hash_tree_root_vector_composite_and_roots_match},
        {"hash_tree_root_list_variants_and_mix_in_length", test_hash_tree_root_list_variants_and_mix_in_length},
        ...
        {"merkleize_progressive_cases", test_merkleize_progressive_cases},
