...
#define ssz_merkleize_progressive(chunks, chunk_count, hash_fn, out_root) \
    ssz_merkleize_progressive(                                            \
        ...
        (hash_fn),                                                        \
        (out_root))
#define ssz_hash_tree_root_progressive_container( \
    field_count,                                  \
    active_fields,                                \
    active_fields_len,                            \
    codec,                                        \
    ...
    ssz_hash_tree_root_progressive_container(     \
        ...
        (hash_fn),                                \
        (out_root))
#define ssz_hash_tree_root_progressive_list_fixed( \
    elements,                                      \
    element_count,                                 \
    element_size,                                  \
    hash_fn,                                       \
...
#define ssz_hash_tree_root_progressive_bitlist(bits_le, bits_le_len, bit_len, hash_fn, out_root) \
    ssz_hash_tree_root_progressive_bitlist(                                                      \
        ...
        (hash_fn),                                                                               \
        (out_root))
#define ssz_hash_tree_root_progressive_container_roots( \
    roots,                                              \
    count,                                              \
    active_fields,                                      \
    active_fields_len,                                  \
...
static bool test_merkleize_progressive_cases(void)
{
    ...
    ASSERT_ERR(
        ssz_merkleize_progressive(((const ssz_chunk_t[]){c0, c1, c2}), 3u, NULL, &root),
        SSZ_SUCCESS);
    ASSERT_CHUNK_EQ(root, expected);
...
static bool test_progressive_hash_tree_root_variants(void)
{
    ...
    ASSERT_ERR(
        ssz_merkleize_progressive(packed_chunks, 2u, NULL, &expected_progressive_list_fixed_data),
        SSZ_SUCCESS);
    ASSERT_ERR(
        ssz_mix_in_length_u64(
            &expected_progressive_list_fixed_data,
            5u,
            NULL,
            &expected_progressive_list_fixed),
        SSZ_SUCCESS);
    ...
    ASSERT_ERR(
        ssz_merkleize_progressive(bit_chunks, 2u, NULL, &expected_bitlist_data),
        SSZ_SUCCESS);
    ASSERT_ERR(
        ssz_mix_in_length_u64(&expected_bitlist_data, 260u, NULL, &expected_bitlist_root),
        SSZ_SUCCESS);
...
static bool test_merkle_additional_error_paths(void)
{
    ...
    ASSERT_ERR(
        ssz_merkleize_progressive(((const ssz_chunk_t[]){make_chunk(0x01u)}), 1u, NULL, NULL),
        SSZ_ERR_INVALID_ARGUMENT);
    ...
    ASSERT_ERR(
        ssz_hash_tree_root_progressive_list_fixed(&one_byte, 1u, 1u, NULL, NULL),
        SSZ_ERR_INVALID_ARGUMENT);
    ASSERT_ERR(
        ssz_hash_tree_root_progressive_list_fixed(&one_byte, 1u, 0u, NULL, &root),
        SSZ_ERR_SCHEMA_INVALID);
#if SIZE_MAX > UINT32_MAX
    ASSERT_ERR(
        ssz_hash_tree_root_progressive_list_fixed(
...
#endif
    ASSERT_ERR(
        ssz_hash_tree_root_progressive_list_fixed(NULL, 1u, 1u, NULL, &root),
        SSZ_ERR_INVALID_ARGUMENT);
#if SIZE_MAX > UINT32_MAX
    ASSERT_ERR(
        ssz_hash_tree_root_progressive_list_fixed(&one_byte, SIZE_MAX, 1u, NULL, &root),
        SSZ_ERR_OVERFLOW);
#endif
    ASSERT_ERR(ssz_hash_tree_root_progressive_list_fixed(NULL, 0u, 1u, NULL, &root), SSZ_SUCCESS);

    ASSERT_ERR(
        ssz_hash_tree_root_progressive_list_composite(1u, NULL, NULL, &root),
        SSZ_ERR_INVALID_ARGUMENT);
    ASSERT_ERR(
        ssz_hash_tree_root_progressive_list_composite(
            1u,
    ...
    ASSERT_ERR(
        ssz_hash_tree_root_progressive_bitlist(&one_byte, 1u, 1u, NULL, NULL),
        SSZ_ERR_INVALID_ARGUMENT);
    ASSERT_ERR(
        ssz_hash_tree_root_progressive_bitlist(&one_byte, 1u, UINT64_MAX, NULL, &root),
        SSZ_ERR_OVERFLOW);
    ASSERT_ERR(
        ssz_hash_tree_root_progressive_bitlist(NULL, 0u, 1u, NULL, &root),
        SSZ_ERR_INVALID_ARGUMENT);
    ASSERT_ERR(
        ssz_hash_tree_root_progressive_bitlist(
            ((const uint8_t[2]){0x00u, 0xFFu}),
            2u,
            9u,
            NULL,
            &root),
...
static bool test_merkle_active_field_validation_errors(void)
{
    ...
    ASSERT_ERR(
        ...
        SSZ_ERR_SCHEMA_INVALID);
    ASSERT_ERR(
        ssz_hash_tree_root_progressive_container(2u, NULL, 1u, &codec, NULL, &root),
        SSZ_ERR_SCHEMA_INVALID);
    ASSERT_ERR(
        ssz_hash_tree_root_progressive_container(
            2u,
    ...
    ASSERT_ERR(
        ssz_hash_tree_root_progressive_container_roots(roots, 2u, NULL, 1u, NULL, &root),
        SSZ_ERR_SCHEMA_INVALID);
    ASSERT_ERR(
        ssz_hash_tree_root_progressive_container_roots(
            roots,
...
static bool test_merkle_failure_propagation_paths(void)
{
    ...
    ASSERT_ERR(
        ssz_merkleize_progressive(merkle_chunks, 2u, &failing_hash, &root),
        SSZ_ERR_HASH_FAILURE);
    ...
    ASSERT_ERR(
        ssz_hash_tree_root_progressive_container(
            ...
            active_fields,
            sizeof(active_fields),
            (&(ssz_member_codec_t){.ctx = (void *)&map,
                                   .write = NULL,
                                   .read = NULL,
                                   .root = root_map_root}),
            &failing_hash,
    ...
    ASSERT_ERR(
        ...
        SSZ_ERR_HASH_FAILURE);
    ASSERT_ERR(
        ssz_hash_tree_root_progressive_list_roots(merkle_chunks, 2u, &failing_hash, &root),
        SSZ_ERR_HASH_FAILURE);
