...
void process_serialized_file(const char *folder_name, const char *serialized_file_path)
{
    ...
    printf("\nFolder: %s | Max Bits: %d | Actual Bits: %zu | Variant: %s\n", folder_name, max_bits, actual_bit_count, variant);
    ...
    ssz_error_t pack_err = ssz_pack_bits(in_mem, actual_bit_count, packed_chunks, &packed_chunk_count);
    ...
    {
        fprintf(stderr, "Mix-in length error in folder %s: %d\n", folder_name, mix_err);
        free(packed_chunks);
        free(out_buf);
        free(expected_data);
        free(in_mem);
