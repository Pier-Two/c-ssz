...
static bool parse_case_descriptor(
    const char *case_name,
    vector_elem_kind_t *out_kind,
    uint64_t *out_length,
    size_t *out_elem_size)
{
    char type_name[32];
    unsigned long long parsed_length = 0ULL;

    if ((case_name == NULL) || (out_kind == NULL) || (out_length == NULL) || (out_elem_size == NULL))
    {
        return false;
    }

    if (sscanf(case_name, "vec_%31[^_]_%llu", type_name, &parsed_length) != 2)
    {
        return false;
    }
    ...
    *out_length = (uint64_t)parsed_length;
    return true;
}

static bool load_expected_vector(
    const char *value_path,
    vector_elem_kind_t kind,
    uint64_t length,
    size_t elem_size,
    uint8_t **out_bytes,
    size_t *out_len)
{
    ...
    *out_len = 0u;
    ...
    if (length > (uint64_t)SIZE_MAX / elem_size)
    {
        return false;
    }

    total_len = (size_t)length * elem_size;

    doc = yaml_parse_file(value_path);
    if ((doc == NULL) || (doc->type != YAML_NODE_SEQUENCE) || (doc->as.sequence.count != (size_t)length))
    {
        yaml_node_free(doc);
        return false;
    }
    ...
    for (size_t i = 0u; i < (size_t)length; i++)
    {
        ...
        if ((kind == VECTOR_ELEM_UINT8) || (kind == VECTOR_ELEM_UINT16) ||
            (kind == VECTOR_ELEM_UINT32) || (kind == VECTOR_ELEM_UINT64))
        {
            uint64_t value = 0u;
            ...
            else if (kind == VECTOR_ELEM_UINT16)
            {
                if (value > UINT16_MAX)
                {
                    free(bytes);
            ...
            else if (kind == VECTOR_ELEM_UINT32)
            {
                if (value > UINT32_MAX)
                {
                    free(bytes);
...
static ssz_error_t deserialize_vector_checked(
    vector_elem_kind_t kind,
    ...
    uint8_t *out)
{
    ...
    if ((serialized == NULL) || (out == NULL) || (elem_size == 0u))
    {
    ...
    expected_len = (size_t)length * elem_size;

    if (kind != VECTOR_ELEM_BOOL)
    {
        return ssz_deserialize_vector_fixed(
            serialized,
            serialized_len,
            length,
            elem_size,
...
static void run_valid_case(spec_report_t *report, const char *suite_dir, const char *case_name)
{
    ...
    if (!parse_case_descriptor(case_name, &kind, &length, &elem_size))
    {
        report->valid_failed++;
        spec_report_record_failure(report, case_path, "could not parse vector descriptor");
        goto done;
    }
    ...
    if (!load_expected_vector(value_path, kind, length, elem_size, &expected, &expected_len))
    {
        report->valid_failed++;
    ...
    if (deserialize_vector_checked(kind, serialized, serialized_len, length, elem_size, decoded) != SSZ_SUCCESS)
    {
        report->valid_failed++;
    ...
    if ((expected_len != serialized_len) || (memcmp(decoded, expected, expected_len) != 0))
    {
        report->valid_failed++;
    ...
    if (ssz_serialize_vector_fixed(decoded, length, elem_size, NULL, 0u, &reencoded_len) != SSZ_SUCCESS)
    {
        report->valid_failed++;
    ...
    if (ssz_serialize_vector_fixed(decoded, length, elem_size, reencoded, reencoded_len, &reencoded_len) !=
        SSZ_SUCCESS)
    {
        report->valid_failed++;
    ...
    if (ssz_hash_tree_root_vector_fixed(decoded, length, elem_size, ssz_hash_default(), &actual_root) !=
        SSZ_SUCCESS)
    {
        report->valid_failed++;
...
static void run_invalid_case(spec_report_t *report, const char *suite_dir, const char *case_name)
{
    ...
    {
        report->invalid_failed++;
        spec_report_record_failure(report, case_path, "invalid descriptor in test case");
        goto done;
    }
    ...
    err = deserialize_vector_checked(kind, serialized, serialized_len, length, elem_size, decoded);
    if (err == SSZ_SUCCESS)
    {
        report->invalid_failed++;
...
int main(void)
{
    ...
    {
        int rc = spec_report_print("ssz_generic/basic_vector", &report);
