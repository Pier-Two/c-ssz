...
#ifndef TESTS_DIR
#define TESTS_DIR "tests/fixtures/general/phase0/ssz_generic/progressive_bitlist"
#endif
...
static void run_valid_case(spec_report_t *report, const char *suite_dir, const char *case_name)
{
    ...
    bitfield_cap = serialized_len == 0u ? 1u : serialized_len;
    ...
    if (ssz_hash_tree_root_progressive_bitlist(bits, bitfield_cap, bit_len, ssz_hash_default(),
...
int main(void)
{
    ...
    {
        int rc = spec_report_print("ssz_generic/progressive_bitlist", &report);
