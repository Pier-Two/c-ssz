...
static bool parse_case_size(const char *case_name, uint64_t *out_size)
{
    unsigned long long parsed = 0ULL;
    if ((case_name == NULL) || (out_size == NULL))
    {
        return false;
    }
    if (sscanf(case_name, "bitvec_%llu", &parsed) != 1)
    {
        return false;
    }
    *out_size = (uint64_t)parsed;
    return true;
}
...
static void run_valid_case(spec_report_t *report, const char *suite_dir, const char *case_name)
{
    ...
    if (!parse_case_size(case_name, &bit_count) || !bits_to_bytes(bit_count, &bitfield_len))
    {
        report->valid_failed++;
        spec_report_record_failure(report, case_path, "could not parse bitvector size");
        goto done;
    }
    ...
    if (ssz_deserialize_bitvector(serialized, serialized_len, bit_count, bits, bitfield_len) != SSZ_SUCCESS)
    {
        report->valid_failed++;
    ...
    if (ssz_serialize_bitvector(bits, bitfield_len, bit_count, NULL, 0u, &reencoded_len) != SSZ_SUCCESS)
    {
        report->valid_failed++;
    ...
    if (ssz_serialize_bitvector(bits, bitfield_len, bit_count, reencoded, reencoded_len, &reencoded_len) !=
        SSZ_SUCCESS)
    {
        report->valid_failed++;
    ...
    if (ssz_hash_tree_root_bitvector(bits, bitfield_len, bit_count, ssz_hash_default(), &actual_root) !=
        SSZ_SUCCESS)
    {
        report->valid_failed++;
...
static void run_invalid_case(spec_report_t *report, const char *suite_dir, const char *case_name)
{
    ...
    err = ssz_deserialize_bitvector(serialized, serialized_len, bit_count, bits, bitfield_len);
    if (err == SSZ_SUCCESS)
    {
        report->invalid_failed++;
...
int main(void)
{
    ...
    {
        int rc = spec_report_print("ssz_generic/bitvector", &report);
